...
The resulting block model will have its core region set by the object and limited by the `Core depth` parameter.
The core region will contain cells at the requested `Mininum cell size` in all three Cartesian directions. `Horizontal` and `Vertical padding` will be applied outward from the
